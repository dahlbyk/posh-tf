...
if($PSVersionTable.PSVersion.Major -lt 2) {
    Write-Warning "posh-tf requires PowerShell 2.0 or better; you have version $($Host.Version)."
    return
}
...
if(Select-String -Path $PROFILE -Pattern $profileLine -Quiet -SimpleMatch) {
    Write-Host "It seems posh-tf is already installed..."
    return
}

Write-Host "Adding posh-tf to profile..."
@"

# Load posh-tf example profile
$profileLine

"@ | Out-File $PROFILE -Append -WhatIf:$WhatIf -Encoding (Get-FileEncoding $PROFILE)

Write-Host 'posh-tf sucessfully installed!'
